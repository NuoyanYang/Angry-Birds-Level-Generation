class txt2xml(object):
    ...
    def txt2xml(self, text):
        ...
        for vec in vector:
            ...
            if "\n" in vec:
                ...
                for x_median, key, rotation, object_name, material in objects:
                    ...
                    text, pigs, tnts = self.create_text(
        ...
        texts += "</GameObjects>\n"
        texts += "</Level>\n"
        return texts, pig_count, tnt_count
    ...
    def fitness_cal(self, vector):
            ...
            if vec == "<unk>":
                ...
                for i, v in enumerate(vec):
                    ...
                    if v != 0:
                        ...
                        if object_name == "TNT":
                            tnt_count += 1
                        elif object_name == "Pig":
                            pig_count += 1

                        if object_name in ["TNT", "Pig"]:
                            material = "BasicSmall"
                            material_set.add(material)
                            object_set.add(object_name)
                            object_count += 1
                        elif object_name not in ["Platform", "empty"]:
                            # Ignore platform and empty(air block)
                            material = self.code2material[(v - 4) // 19]
                            material_set.add(material)
                            object_set.add(object_name)
                            object_count += 1
        ...
        return pig_count, tnt_count, len(material_set), len(object_set), object_count